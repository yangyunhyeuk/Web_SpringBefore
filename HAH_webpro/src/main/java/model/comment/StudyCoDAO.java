...
public class StudyCoDAO {
	...
	public ArrayList<StudyCoVO> SelectAll() {
		...
		try {
			String sql_SelectAll = "SELECT * FROM STUDYCOMMENT"; // 최근 게시글 상단 배치
	...
	public StudyCoVO SelectOne(StudyCoVO vo) {
		...
		try {
			String sql_SelectOne = "select * from STUDYCOMMENT where cnum = ?"; // 일단 기본키인 cnum으로 조건절 작성
	...
	public boolean UpdateDB(StudyCoVO vo) {
		...
		PreparedStatement pstmt = null;
		try {
			String sql_UPDATE = "UPDATE STUDYCOMMENT SET COMM=?, PNUM=?, MNAME=?, MID=? where CNUM=?"; // 업데이트 조건, cnum
